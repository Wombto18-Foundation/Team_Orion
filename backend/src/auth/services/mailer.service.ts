...
export class MailerService {
  ...
  async sendWelcomeVolunteerEmail(params: { email: string; name: string; donorId: string; volunteerId?: string }): Promise<void> {
    const frontendUrl = this.configService.get<string>('FRONTEND_URL') || 'http://localhost:5173';
    const loginUrl = `${frontendUrl}/login`;
  ...
  async sendWelcomePartnerEmail(params: { email: string; contactPerson: string; organizationName: string; partnerId: string }): Promise<void> {
  ...
  private buildWelcomeEmail(params: {
    ...
    dashboardFeatures: { icon: string; title: string; desc: string }[];
  }): string {
